...
class DecisionTree(ClassifierMixin, BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        if self.tree_init_mode == "train":
            from ShrubEnsembles.ShrubEnsembles import DecisionTree_TRAIN
            self.model = DecisionTree_TRAIN(self.n_classes_, self.max_depth, self.max_features, self.seed, self.step_size)
        else:
            from ShrubEnsembles.ShrubEnsembles import DecisionTree_RANDOM
            self.model = DecisionTree_RANDOM(self.n_classes_, self.max_depth, self.max_features, self.seed, self.step_size)
