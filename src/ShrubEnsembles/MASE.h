...
class MASE : public TreeEnsemble<data_t, loss_type, opt, tree_opt> {
...
public:
    ...
    ) : TreeEnsemble<data_t, loss_type, opt, tree_opt>(n_classes, max_depth, seed, false, max_features, step_size, ENSEMBLE_REGULARIZER::TYPE::NO, 0, TREE_REGULARIZER::TYPE::NO, 0), n_trees(n_trees), n_rounds(n_rounds), batch_size(batch_size), bootstrap(bootstrap), burnin_steps(burnin_steps), n_worker(n_worker), init_tree_size(init_tree_size) { 
    }
    ...
    void next(matrix2d<data_t> const &X, matrix1d<unsigned int> const &Y, unsigned int n_parallel, bool boostrap, unsigned int batch_size, unsigned int burnin_steps) {
        std::vector<TreeEnsemble<data_t, loss_type, opt, tree_opt>> ses(n_parallel, *this);
