...
public:
    ShrubEnsembleAdaptor(
        ...
        if (tree_init_mode == "random" && tree_update_mode == "incremental") {
            model = new ShrubEnsemble<TREE_INIT::RANDOM, TREE_NEXT::INCREMENTAL, data_t>( n_classes, max_depth, seed, normalize_weights, burnin_steps, max_features, LOSS::from_string(loss), step_size, from_string(step_size_mode), ENSEMBLE_REGULARIZER::from_string(ensemble_regularizer), l_ensemble_reg, TREE_REGULARIZER::from_string(tree_regularizer), l_tree_reg, l_l2_reg );
...
py::class_<ShrubEnsembleAdaptor>(m, "CShrubEnsembleBindings")
    .def(py::init<unsigned int, unsigned int,unsigned long, bool, unsigned int, unsigned int, std::string, data_t, std::string, std::string, data_t, std::string, data_t, data_t, std::string, std::string>(), py::arg("n_classes"), py::arg("max_depth"), py::arg("seed"), py::arg("normalize_weights"), py::arg("burnin_steps"), py::arg("max_features"), py::arg("loss"), py::arg("step_size"), py::arg("step_size_mode"), py::arg("ensemble_regularizer"), py::arg("l_ensemble_reg"), py::arg("tree_regularizer"), py::arg("l_tree_reg"), py::arg("l_l2_reg"), py::arg("tree_init_mode"), py::arg("tree_update_mode"))
