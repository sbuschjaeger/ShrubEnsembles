...
class GAShrubEnsemble : public GAShrubEnsembleInterface, private ShrubEnsemble<OPTIMIZER::OPTIMIZER_TYPE::NONE, tree_opt, tree_init> {
...
public:
    ...
    void fit(std::vector<std::vector<data_t>> const &X, std::vector<unsigned int> const &Y) {
        ShrubEnsemble<OPTIMIZER::OPTIMIZER_TYPE::NONE, tree_opt, tree_init>::fit_gd(X,Y,n_trees,bootstrap,init_batch_size,n_rounds,n_batches);
    }

    void next(std::vector<std::vector<data_t>> const &X, std::vector<unsigned int> const &Y) {
        ShrubEnsemble<OPTIMIZER::OPTIMIZER_TYPE::NONE, tree_opt, tree_init>::next_gd(X,Y,n_batches);
    }


    void init(std::vector<std::vector<data_t>> const &X, std::vector<unsigned int> const &Y) {
        ShrubEnsemble<OPTIMIZER::OPTIMIZER_TYPE::NONE, tree_opt, tree_init>::init_trees(X,Y,n_trees,bootstrap,init_batch_size);
    }
