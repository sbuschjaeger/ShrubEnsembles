...
template <LOSS::TYPE loss_type, OPTIMIZER::OPTIMIZER_TYPE opt, TREE_INIT tree_init>
class OnlineShrubEnsemble : public OnlineShrubEnsembleInterface, private ShrubEnsemble<loss_type, opt, OPTIMIZER::OPTIMIZER_TYPE::NONE, tree_init> {
...
public:
    ...
    ) : ShrubEnsemble<loss_type, opt, OPTIMIZER::OPTIMIZER_TYPE::NONE, tree_init>(
            n_classes,
            max_depth,
            seed,
            normalize_weights,
    ...
    void next(std::vector<std::vector<data_t>> const &X, std::vector<unsigned int> const &Y) {
        ShrubEnsemble<loss_type, opt, OPTIMIZER::OPTIMIZER_TYPE::NONE, tree_init>::next(X,Y);
    }
    ...
    void init(std::vector<std::vector<data_t>> const &X, std::vector<unsigned int> const &Y, unsigned int n_trees, bool bootstrap, unsigned int batch_size) {
        ShrubEnsemble<loss_type, opt, OPTIMIZER::OPTIMIZER_TYPE::NONE, tree_init>::init_trees(X,Y,n_trees,bootstrap,batch_size);
    }

    std::vector<std::vector<data_t>> predict_proba(std::vector<std::vector<data_t>> const &X) {
        return ShrubEnsemble<loss_type, opt, OPTIMIZER::OPTIMIZER_TYPE::NONE, tree_init>::predict_proba(X);
    }
    
    std::vector<internal_t> weights() const {
        return ShrubEnsemble<loss_type, opt, OPTIMIZER::OPTIMIZER_TYPE::NONE, tree_init>::weights();
    }

    unsigned int num_trees()const {
        return ShrubEnsemble<loss_type, opt, OPTIMIZER::OPTIMIZER_TYPE::NONE, tree_init>::num_trees();
    }

    unsigned int num_bytes()const {
        return ShrubEnsemble<loss_type, opt, OPTIMIZER::OPTIMIZER_TYPE::NONE, tree_init>::num_bytes();
    }
    
    unsigned int num_nodes() const {
        return ShrubEnsemble<loss_type, opt, OPTIMIZER::OPTIMIZER_TYPE::NONE, tree_init>::num_nodes();
    }
};
